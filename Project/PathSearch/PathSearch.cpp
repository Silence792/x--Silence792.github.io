		...
		void PathSearch::initialize(TileMap* _tileMap)
		{
			...
			for (int i = 0; i < rowCount; i++)
			{
				for (int j = 0; j < colCount; j++)
				{
					...
					{
						//create node and jam it in the map
						Tile* tile = _tileMap->getTile(i, j);
						//tile->setFill(COLOR_BLUE);
			...
			for (int i = 0; i < rowCount; i++)
			{
				for (int j = 0; j < colCount; j++)
				{
					if (_tileMap->getTile(i, j)->getWeight() != 0)
					{
						...
						{
							SearchNode* node = map[_tileMap->getTile(i + 1, j)];
							Edge* edge = new Edge;// (node);
							edge->endpoint = node;
							myNode->edges.push_back(edge);

							//curTile->addLineTo(node->tileLoc, COLOR_RED);
						}
						if ((i - 1 >= 0) && (j - 1 >= 0) && areAdjacent(curTile, _tileMap->getTile(i - 1, j - 1)))
						{
							SearchNode* node = map[_tileMap->getTile(i - 1, j - 1)];
							Edge* edge = new Edge;// (node);
							edge->endpoint = node;
							myNode->edges.push_back(edge);
							//curTile->addLineTo(node->tileLoc, COLOR_RED);
						...
						{
							SearchNode* node = map[_tileMap->getTile(i, j - 1)];
							Edge* edge = new Edge;// (node);
							edge->endpoint = node;
							myNode->edges.push_back(edge);
							//curTile->addLineTo(node->tileLoc, COLOR_RED);
